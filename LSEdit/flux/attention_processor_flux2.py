...
class Flux2AttnProcessor:
    ...
    def _maybe_replace_double_stream_values(self, value):
        ...
        if v_pred.shape[2] == 0 or v_ref.shape[2] == 0 or v_pred.shape[2] != v_ref.shape[2]:
            return value
        replaced, scores = self._replace_latent_values_with_scores(v_pred, v_ref)
        refined = self._apply_bhc(replaced, v_ref, scores) if scores is not None else replaced
        pred_start = self.state.text_n
...
def enable_headwise_subjectbg_subjectrelease_havedit(pipeline, config, **_kwargs):
    ...
    original_processors = []
    for block, _, _ in adapter.iter_transformer_blocks(pipeline):
    ...
    for block, block_name, stream_kind in adapter.iter_transformer_blocks(pipeline):
        ...
        processor = (
            build_attention_processor(
                ...
                default_processor_cls=HeadwiseSubjectBackgroundSubjectReleaseFluxAttnProcessor,
            )
            if callable(build_attention_processor)
            else HeadwiseSubjectBackgroundSubjectReleaseFluxAttnProcessor(
                state=state,
                block_name=block_name,
